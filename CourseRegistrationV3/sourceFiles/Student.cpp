...
int Student::getTermCreditHours() const {
    return termCreditHours;
}
...
const unordered_map<string, int>& Student::getGrades() const {
    return grades;
}

bool Student::hasCompletedCourse(const string& courseCode) const {
    return completedCourses.find(courseCode) != completedCourses.end();
}

// للتحقق مما إذا كان مقرر معين ضمن المقررات المسجلة
bool Student::isRegisteredInCourse(const string& courseCode) const {
    return registeredCourses.find(courseCode) != registeredCourses.end();
}
...
void Student::setTermCreditHours(int hours) {
    this->termCreditHours = hours;
}
...
void Student::viewCourses() {
    ...
    for (auto it = courses.begin(); it != courses.end(); it++) {
        cout << "Code: " << it->second.getCode() << "\t" << "name: " << it->second.getName()
            << endl;
...
void Student::RegisterForCourse(string code) {
    ...
    if (checkprerequisites && this->getTermCreditHours() >= courses[code].getCreditHours()) {
        this->addRegisteredCourse(code);
        this->setTermCreditHours(this->getTermCreditHours() - courses[code].getCreditHours());
        cout << "Successfully registered for course: " << courses[code].getName() << endl;
    }
    else {
        cout << "You can't be registered to this course. You either have to complete "
...
void Student::viewGrades() {
    ...
    for (const auto& grade_pair : getGrades()) {
        ...
        if (courses.find(courseCode) != courses.end()) {
            ...
            cout << course.getName() << " -> (Grade: " << grade << ") (Semester: "
                << course.getSemester() << ")\n";
        }
        else {
            cout << courseCode << " -> (Grade: " << grade << ") (Course details not available)\n";
        }
    }
    goBack();
}
...
void Student::generateReports() {
    ...
    for (const auto& courseCode : getCompletedCourses()) {
        if (getGrades().find(courseCode) != getGrades().end() && courses.find(courseCode) != courses.end()) {
            cout << courseCode << ": " << getGrades().at(courseCode) << " (Semester: "
                << courses[courseCode].getSemester() << ")\n";
        }
    }
