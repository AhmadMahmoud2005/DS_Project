...
string Course::getInstractorName() const {
...
float Course::getCreditHours() const {
    return credit_hours;
}
...
void Course::setSyllabus(const string& syllabus) {
...
void Course::setCreditHours(float hours) {
    this->credit_hours = hours;
}

void Course::addPrerequisite(const string& code) {
    prerequisites.insert(code);
}
bool Course::hasPrerequisite(const string& courseCode) const {
    return prerequisites.find(courseCode) != prerequisites.end();
}
